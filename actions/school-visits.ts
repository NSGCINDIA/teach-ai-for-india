...
import { logSchoolVisitSchema, updateSchoolVisitSchema } from '@/lib/validations/school-visits'
...
/**
 * Edit the one existing visit record for a school. Same role gate as logging
 * it (Campus/Outreach Lead, own campus, + admin); doesn't touch the pipeline
 * status — that already advanced when the visit was first logged.
 */
export async function updateSchoolVisit(
  ...
  const parsed = updateSchoolVisitSchema.safeParse(Object.fromEntries(formData))
  if (!parsed.success) return { error: parsed.error.issues[0].message, values }

  const supabase = await createClient()
  const { error } = await supabase.rpc('update_school_visit', {
...
function humanizeDbError(msg: string): string {
  ...
  if (/school_visits_school_id_key|duplicate key/i.test(msg))
    return 'A visit has already been logged for this school — edit it instead.'
  return msg
}
