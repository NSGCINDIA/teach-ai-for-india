  ...
  return (
    <section className="tai-section bg-background">
      <div className="tai-container-wide px-5 md:px-8 lg:px-12">
        ...
        <Reveal delay={0.1}>
          <p className="tai-reading mt-4 text-lg text-muted-foreground">{content.supportingLine}</p>
        </Reveal>
          ...
          <div className="relative pl-10 lg:col-span-7">
            ...
            {content.steps.map((step, i) => (
              <Reveal key={step.title} delay={i * 0.08} className="relative pb-10 last:pb-0">
                <span className="absolute -left-10 top-0.5 grid size-7 place-items-center rounded-full border-2 border-[var(--tai-crimson)] bg-[var(--tai-paper)] text-xs font-bold text-[var(--tai-crimson)]">
                  {i + 1}
                </span>
        ...
        <Reveal delay={0.2}>
          <p className="tai-prose mt-16 border-l border-border pl-6 text-base italic leading-relaxed text-muted-foreground">
            {volunteerNote.body}
          </p>
        </Reveal>
      </div>
