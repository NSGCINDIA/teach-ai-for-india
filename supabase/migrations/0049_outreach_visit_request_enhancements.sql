-- 0049_outreach_visit_request_enhancements.sql
-- Adds priority, expected_outcomes, and transportation fields to outreach_visit_requests.
-- Removes the "purpose" field from the create RPC (purpose column made nullable for
-- backward compatibility with any existing rows).

-- ─── Schema changes ───────────────────────────────────────────────────────────
ALTER TABLE outreach_visit_requests
  ...
  ADD COLUMN IF NOT EXISTS transportation    text CHECK (transportation IN ('Bike','Car','Auto'));

-- Make purpose nullable so existing rows are unaffected and the new RPC can omit it.
ALTER TABLE outreach_visit_requests
  ALTER COLUMN purpose DROP NOT NULL;

COMMENT ON COLUMN outreach_visit_requests.priority          IS 'Visit urgency: High, Medium, or Low.';
COMMENT ON COLUMN outreach_visit_requests.expected_outcomes IS 'Objectives for the visit (multi-select array).';
COMMENT ON COLUMN outreach_visit_requests.transportation    IS 'Mode of transport for the visit team: Bike, Car, or Auto.';

-- ─── Updated RPC ──────────────────────────────────────────────────────────────
-- Drop old 5-arg signature first, then create the new one without p_purpose.
DROP FUNCTION IF EXISTS public.create_outreach_visit_request(uuid, text, date, numeric, uuid[]);
-- Also drop the 8-arg version from a partial earlier run, if it exists.
DROP FUNCTION IF EXISTS public.create_outreach_visit_request(uuid, text, date, numeric, uuid[], text, text[], text);
...
COMMENT ON FUNCTION public.create_outreach_visit_request(uuid, date, numeric, uuid[], text, text[], text) IS
  'File an outreach visit request (priority + expected outcomes required, transportation optional); notifies Campus Lead + Finance Lead.';
